...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.loginTenant(username, password);
      router.push("/");
    } catch (err: unknown) {
    ...
    >
      <div className="glass-card" style={{ width: "100%", maxWidth: "420px", padding: "40px" }}>
        <div style={{ marginBottom: "32px", textAlign: "center" }}>
          ...
          <p style={{ color: "var(--text-muted)", fontSize: "0.9rem", marginTop: "6px" }}>Удирдлагын системд нэвтрэх</p>
        </div>
        ...
        <div style={{ marginTop: "24px", fontSize: "0.85rem", color: "var(--text-muted)", textAlign: "center" }}>
          Системийн админ уу? <Link href="/superadmin/login" style={{ color: "#c084fc", textDecoration: "none", fontWeight: 600 }}>SuperAdmin Login</Link>
        </div>
      </div>
    </div>
  );
